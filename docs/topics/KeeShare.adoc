...
1. Go to Tools &gt; Settings. Select the KeeShare category on the left sidebar.
2. Check _Allow import_ if you want to import shared credentials. Check _Allow export_ if you want to share credentials.
3. (Optional) Click _Generate_ to create your own certificate or _Import_ to select an existing one. The certificate allows you to sign shared databases. This ensures the integrity of the share and prevent import of untrusted information.
